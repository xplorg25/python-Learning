...
def getConnection():
    ...
    connection_string = f"mongodb://{db_ip}:{port}/{authSrc}?authSource={authSrc}"
...
def process_day(date_str, collection, to_save_coll, logger, tc):
    if not date_str:
        return 0
    ...
    try:
        cursor = collection.find(
            {"hwNmNorthboundEventDate": date_str, "SynchReceiver": "N"},
        ...
        # Store metrics for sharding strategy
        daily_metrics = {}
        ...
        for h_doc in cursor:
            ...
            if h_doc["hwNmNorthboundFaultFlag"] == "Fault" or h_doc["hwNmNorthboundFaultFlag"] == "Recovery":
                ...
                query = {
                    "hwNmNorthboundEventDate": {"$gte": date1_str, "$lte": date2_str},
                    "hwNmNorthboundEventTime": {"$gte": filtered_time1, "$lte": filtered_time2},
                    "hwNmNorthboundFaultFlag": "Fault", "SynchReceiver": "N"
                }

                if rec:
                    query["hwNmNorthboundFaultFlag"] = "Recovery"

                self_cursor = collection.find(
                    query,
                    {"hwNmNorthboundEventDate": 1, "hwNmNorthboundEventTime": 1, "hwNmNorthboundEventName": 1,
                     "hwNmNorthboundObjectInstance": 1, "hwNmNorthboundNEName": 1}
                )

                inner_dict = defaultdict(int)
                for res in self_cursor:
                    key2 = str(res["hwNmNorthboundNEName"].replace('.', r'\u002e') +
                               "**" + res["hwNmNorthboundObjectInstance"].replace('.', r'\u002e') +
                               "**" + res["hwNmNorthboundEventName"].replace(".", r"\u002e"))
                    inner_dict[key2] += 1
                ...
                # SOLUTION: Split documents by using a compound key or additional shard key
                # Add a month or week identifier to shard the data
                month_key = date_obj.strftime('%Y-%m')  # Use year-month as a shard

                # Create a compound primary key
                compound_key = f"{key_h}:{month_key}"

                # Prepare update operation with the compound key
                update_query = {"key1": compound_key}

                # Store metrics to analyze overall data volume
                if key_h not in daily_metrics:
                    daily_metrics[key_h] = 0
                daily_metrics[key_h] += sum(inner_dict.values())
                ...
                # Add metadata to make data retrieval easier
                update_values["$set"] = {
                    "original_key": key_h,
                    "period": month_key,
        ...
        # Log high-volume keys that might need attention
        high_volume_keys = {k: v for k, v in daily_metrics.items() if v > 10000}
        if high_volume_keys:
            logger.warning(f"High volume keys detected: {high_volume_keys}")
...
def ensure_indexes(collections, logger):
    ...
    try:
        ...
        # Collection indexes firstTimeDetectedDate_1_firstTimeDetectedTime_1_severity_1
        if "hwNmNorthboundEventDate_1_hwNmNorthboundFaultFlag_1" not in collections["huawei_ipran_alarms"].index_information():
            logger.info("Creating index on hia collection...")
            collections["huawei_ipran_alarms"].create_index([
        ...
        if "hashed_key1" not in collections["ZambiaTesting_huawei_to_huawei"].index_information():
            logger.info("Creating hashed index on test_collection...")
            collections["ZambiaTesting_huawei_to_huawei"].create_index([("key1", "hashed")], name="hashed_key1")
...
def main():
    # Setup logging
    logger = setup_logging(f"{__name__}.log")
    ...
    try:
        ...
        db = connection["nokia_zambia_project_telco360_topology"]
        collection = db["huawei_ipran_alarms"]


        to_save = connection["Corel_test"]
        to_save_coll = to_save["ZambiaTesting_huawei_to_huawei"]

        # Ensure all needed indexes exist
        ensure_indexes({
            "huawei_ipran_alarms" : collection,
            "ZambiaTesting_huawei_to_huawei" : to_save_coll
        }, logger)

        # Get distinct dates - use a cursor to avoid loading all into memory
        logger.info("Retrieving distinct dates for processing")
        # distinct_dates = sorted([d for d in collection.distinct("firstTimeDetectedDate") if d])
        distinct_dates= []
        for d in collection.distinct("hwNmNorthboundEventDate"):
        ...
        for i, date_str in enumerate(distinct_dates):
            ...
            count = process_day(date_str, collection, to_save_coll, logger,tc)
