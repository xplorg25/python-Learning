...
def process_day(date_str, collection, to_save_coll, logger,tc):
    ...
    try:
        cursor = collection.find( #**change
            {"hwNmNorthboundEventDate": date_str , "SynchReceiver":"N" , "hwNmNorthboundEventName":{"$ne":"BFD session down"}},
        ...
        # cursor = collection.find({"firstTimeDetectedDate": date_str ,"severity":{"$ne":"Info"}},
        #     {"firstTimeDetectedDate": 1, "firstTimeDetectedTime": 1, "alarmName": 1, "severity": 1,
        ...
        for h_doc in cursor:
            ...
            if h_doc["hwNmNorthboundFaultFlag"] == "Fault" or h_doc["hwNmNorthboundFaultFlag"] == "Recovery":
                if h_doc["hwNmNorthboundFaultFlag"] == "Recovery": #**change
                    rec = True
                # neName = h_doc["nodeName"]
                # managedObj = h_doc["objectFullName"]
                # alarmName = h_doc["alarmName"]
                # time_str = h_doc["firstTimeDetectedTime"]
                # date_str = h_doc["firstTimeDetectedDate"]
                ...
                #     "hwNmNorthboundFaultFlag": "Fault"
                # }

                query = {
                    "hwNmNorthboundEventDate": {"$gte": date1_str, "$lte": date2_str}, "hwNmNorthboundEventName":{"$ne":"BFD session down"},
                    "hwNmNorthboundEventTime": {"$gte": filtered_time1, "$lte": filtered_time2},
