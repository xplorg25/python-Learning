        ...
        for h_doc in cursor:
            ...
            if h_doc["severity"]:
                ...
                query = {
                    ...
                    "hwNmNorthboundEventName" : {"$ne" : "BFD session down"}
                }
                ...
                hua_cursor = collection2.find(
                    query,
                    {"hwNmNorthboundEventDate": 1, "hwNmNorthboundEventTime": 1, "hwNmNorthboundEventName": 1,
                     "hwNmNorthboundObjectInstance": 1, "hwNmNorthboundNEName": 1},no_cursor_timeout=True
                )

                inner_dict = defaultdict(int)
                for res in hua_cursor:
...
def ensure_indexes(collections, logger):
    ...
    try:
        ...
        if "hwNmNorthboundEventDate_1_hwNmNorthboundEventDate_1_hwNmNorthboundFaultFlag_1" not in collections[
            "huawei_ipran_alarms"].index_information():
            logger.info("Creating index on hia collection...")
            collections["huawei_ipran_alarms"].create_index([
                ("hwNmNorthboundEventDate", 1),
                ("hwNmNorthboundEventTime", 1),
                ("hwNmNorthboundFaultFlag", 1)
            ])
            logger.info("Index created successfully on hua collection")
        ...
        # if compound_index_name not in collections["sam_microwave_alarms"].index_information():
        #     logger.info("Creating index on sma collection...")
        #     collections["sam_microwave_alarms"].create_index([
        ...
        compound_index_name = "firstTimeDetectedDate_1_severity_1"
        if compound_index_name not in collections["sam_microwave_alarms"].index_information():
            logger.info("Creating index on sma collection...")
            collections["sam_microwave_alarms"].create_index([
                ("firstTimeDetectedDate", 1),
        ...
        if "hashed_key1" not in collections["ZAMBIATesting_sam_to_huawei"].index_information():
            logger.info("Creating hashed index on test_ collection...")
            collections["ZAMBIATesting_sam_to_huawei"].create_index([("key1", "hashed")], name="hashed_key1")
...
def main():
    ...
    try:
        ...
        collection = db["sam_microwave_alarms"]
        collection2 = db["huawei_ipran_alarms"]

        to_save = connection["Corel_test"]
        to_save_coll = to_save["ZAMBIATesting_sam_to_huawei"]

        # Ensure all needed indexes exist
        ensure_indexes({
            "sam_microwave_alarms" : collection,
            "huawei_ipran_alarms" : collection2,
            "ZAMBIATesting_sam_to_huawei" : to_save_coll
        }, logger)
        ...
        distinct_dates= []
        for d in collection.distinct("firstTimeDetectedDate"):
