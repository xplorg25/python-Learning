...
def ensure_indexes(collections, logger):
    ...
    try:
        ...
        if "hashed_key1" not in collections["ZAMBIATesting_h_to_s"].index_information():
            logger.info("Creating hashed index on test_collection...")
            collections["ZAMBIATesting_h_to_s"].create_index([("key1", "hashed")], name="hashed_key1")
...
def main():
    ...
    try:
        ...
        collection2 = db["sam_microwave_alarms"]

        to_save = connection["Corel_test"]
        to_save_coll = to_save["ZAMBIATesting_h_to_s"]

        # Ensure all needed indexes exist
        ensure_indexes({
            "huawei_ipran_alarms" : collection,
            "sam_microwave_alarms" : collection2,
            "ZAMBIATesting_h_to_s" : to_save_coll
        }, logger)
