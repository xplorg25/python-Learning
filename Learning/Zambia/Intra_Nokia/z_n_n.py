...
def process_day(date_str, collection, to_save_coll, logger,tc):
    ...
    try:
        ...
        cursor = collection.find({"firstTimeDetectedDate": date_str ,"severity": {"$nin": ["Warning", "Cleared"]}, "SynchReceiver":"N"},
        ...
        for h_doc in cursor:
            ...
            if h_doc["severity"]:
                ...
                hua_cursor = collection.find(
                    query,
                    {"firstTimeDetectedDate": 1, "firstTimeDetectedTime": 1, "alarmName": 1,
                     "objectFullName": 1, "nodeName": 1},no_cursor_timeout=True
                )
...
def ensure_indexes(collections, logger):
    ...
    try:
        ...
        compound_index_name = "firstTimeDetectedDate_1_severity_1_SynchReceiver_1"
        if compound_index_name not in collections["sam_microwave_alarms"].index_information():
            logger.info("Creating index on sma collection...")
            collections["sam_microwave_alarms"].create_index([
                ("firstTimeDetectedDate", 1),
                ("severity", 1),("SynchReceiver", 1)
            ])
            logger.info("Index created successfully on sam collection")

        # Output collection index
        if "hashed_key1" not in collections["Zambia_nokia_to_nokia"].index_information():
            logger.info("Creating hashed index on test_ collection...")
            collections["Zambia_nokia_to_nokia"].create_index([("key1", "hashed")], name="hashed_key1")
...
def main():
    ...
    try:
        ...
        to_save_coll = to_save["Zambia_nokia_to_nokia"]

        # Ensure all needed indexes exist
        ensure_indexes({
            "sam_microwave_alarms" : collection,
            "Zambia_nokia_to_nokia" : to_save_coll
        }, logger)
