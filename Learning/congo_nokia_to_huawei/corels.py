...
def ensure_indexes(collections, logger):
    ...
    try:
        ...
        if "hashed_key1" not in collections["CongoTesting_Sam_to_nokia"].index_information():
            logger.info("Creating hashed index on test_ collection...")
            collections["CongoTesting_Sam_to_nokia"].create_index([("key1", "hashed")], name="hashed_key1")
...
def main():
    ...
    try:
        ...
        db = connection["congo_nokia_project_telco360_topology"]
        collection = db["sam_ipran_alarms"]
        collection2 = db["huawei_mpbn_alarms"]

        to_save = connection["Corel_test"]
        to_save_coll = to_save["CongoTesting_Sam_to_nokia"]

        # Ensure all needed indexes exist
        ensure_indexes({
            "sam_ipran_alarms" : collection,
            "huawei_mpbn_alarms" : collection2,
            "CongoTesting_Sam_to_nokia" : to_save_coll
        }, logger)
